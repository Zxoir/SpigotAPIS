package me.zxoir.myapi.cooldownapi;
...
public class Cooldown {
    ...
    public static long setCooldown(Player player, String key, long delay) {
        return calculateRemainder(
                cooldowns.put(player.getUniqueId(), key, System.currentTimeMillis() + (delay * 1000)));
    }
    ...
    public static boolean containsCooldown(Player player, String key) {
        return cooldowns.contains(player.getUniqueId(), key);
    }
    ...
    public static void removeCooldowns(Player player) {
        cooldowns.row(player.getUniqueId()).clear();
    }


    /**
     * Remove a specific cooldown associated with the given player.
     * @param player - the player we will reset.
     */
    public static void removeCooldown(Player player, String key) {
        cooldowns.remove(player.getUniqueId(), key);
    }

    private static long calculateRemainder(Long expireTime) {
        return expireTime != null ? (expireTime - System.currentTimeMillis()) / 1000 : Long.MIN_VALUE;
    }
}
